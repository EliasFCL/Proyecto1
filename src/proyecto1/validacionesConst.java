    ...
    try {
      ...
      if (!declaracion.contains(":") && !declaracion.contains("=")) {
        ...
        if (!Character.isLetter(nombreConstante.charAt(0))) {
          salida.printf("Error. Línea %04d. El nombre de la variable '%s' debe comenzar con una letra.%n", numeroLinea, nombreConstante);
        ...
        if (nombreConstante.isEmpty()) {
          salida.printf("Error 212. Línea %04d. Nombre de constante nulo%n", numeroLinea);
        }

        // Validar "of integer" o "of string"
        if (declaracion.contains(":")) {
